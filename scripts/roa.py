...
NEO_NETWORK_POOL = [ip_network("10.54.0.0/16"), ip_network("fd10:cd54::/32")]
...
def is_neo_network(address):
    return any(
        address.version == ster.version and address.subnet_of(neo)
...
def load_asn(entities: dict):
    for item, entity in iter_toml_file("asn"):
        ...
        assert entity["source"] in ["Buzzster", "DN42", "Internet"]
        yield asn, entity
...
def make_roa_records(roa4, roa6):
    ...
    return "\n".join(["# Buzzster ROA tool", "", *records])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Buzzster ROA tool")
    parser.add_argument(
